...
apply_security_settings()
{
    ...
    if [[ $(node_is_up "$USER_ADMIN_PWD") && ("$USER_ADMIN_PWD" != "$SEED_PASSWORD" || $(elastic_user_exists "$USER_ADMIN_PWD")) ]]; then
      ...
      #update builtin `elastic` account. need to add logic to detect whether return includes HTTP 503 for master_not_discovered_exception b/c min masters not met
